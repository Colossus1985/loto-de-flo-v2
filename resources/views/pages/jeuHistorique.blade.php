...
<div class="card">
    <div class="card-header">
        <div class="d-flex flex-row justify-content-between my-3">
            ...
            <div>
                <a href="{{ route('jouerForm') }}" class="btn btn-light py-0 d-flex align-items-center border border-3" target="_blank">
                    <p>
                        jouer
                    </p>
                </a>
    ...
    <div class="card-body table-responsive bg-light p-2 rounded pt-3">
        <table id="table_jeuxHistory" class="table table-bordered order-column table-hover compact nowrap cell-border small"><?php // Default dataTables  ?>
            <thead>
                <tr>
                    ...
                    <th class="text-center">Montant seul</th>
                ...
                @foreach ($jeux as $jeu)
                    <tr>
                        ...
                        <td class="text-start align-middle">
                            <span>{{ sql2display($jeu->date) }}</span>
                        </td>
                        <td>{{ $jeu->group_name }}</td>

                        <td class="text-end align-middle fw-bold">
                            @if ($jeu->montant == 0)
                                <span>0.00 €</span>
                            @else
                                <span>{{ ifNotZero($jeu->montant, true, ' €', '.', ' ') }} </span>
                            @endif
                        </td>
                        <td class="text-end align-middle fw-bold">
                            @if ($jeu->montant_indi == 0)
                                <span>0.00 €</span>
                            @else
                                <span>{{ ifNotZero($jeu->montant_indi, true, ' €', '.', ' ') }} </span>
                            @endif
                        </td>

                        <td class="text-start align-middle">
                            <span>{{ $jeu->participants }}</span>
                        </td>
                        <td class="text-end align-middle">
                            <span>{{ $jeu->nbPersonnes }}</span>
                        </td>

                        <td class="d-flex justify-content-center align-items-center">
                            <a href="{{ route('delLigGain', $jeu->id)}}">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        lignes = $("#table_jeuxHistory").DataTable({
